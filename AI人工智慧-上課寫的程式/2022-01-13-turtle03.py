...
turtle.speed(9)
turtle.pencolor((116,67,67))    #  設定筆的顏色
turtle.fillcolor('#744343')     #  設定填滿的顏色
...
for i in range(10) :
    # a = random.randint(0,n-1)       #  隨機出一個整數  區間  0 <-> n-1
    item = random.choice(list2)
    turtle.penup()                    #  筆抬起來
    turtle.goto(0,-300+i*30)
    turtle.pendown()                  #  筆放下去

    # turtle.pencolor( list2[a][0] )         #  設定筆的顏色
    # turtle.fillcolor( list2[a][0] )        #  設定填滿的顏色
    turtle.pencolor( item[0] )         #  設定筆的顏色
    turtle.fillcolor( item[0] )        #  設定填滿的顏色
    turtle.begin_fill()
    turtle.circle(300-i*30)
    turtle.end_fill()
