...
for y in range(start_year,end_year+1) :
    for m in range(1,13) :
        if y == start_year and m < start_month :
            ...
            ymd = f'{y}{str(m).zfill(2)}01'
            url = f"https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&date={ymd}&stockNo={stock_no}"
            res = r.get(url)
            stock_json = res.json()
            data = stock_json['data']
            list_2D_t = 整理股票資料(data)
            list_2D += list_2D_t
...
myfont_10 = mpl.font_manager.FontProperties(fname=r'C:\Windows\Fonts\calibrib.TTF',size=10)
